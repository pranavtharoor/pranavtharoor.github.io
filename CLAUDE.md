...
- **Print** (`@media print` at the end of the `<style>`): `Cmd+P` is the CV — that is why there is no `Resume.pdf`. Ink on white, film/nav/marquee/strip dropped, link targets printed via `a::after { content: attr(href) }`. One trap when editing: anything coloured `--bone` vanishes on white (`.hero-top .mono span` had to be forced dark). To preview it, `sed 's/@media print {/@media screen {/' index.html > _tmp.html`, look at that, then delete it.
